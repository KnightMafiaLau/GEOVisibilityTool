...
6. **品牌识别** — 直接问品牌本身："X 是什么？X 做什么产品？X 的核心技术 / 团队 / 客户群是什么？"。用来测 LLM 有没有收录这个品牌、收录得对不对、有没有跟同名/类似品牌串掉。
...
1. 品牌名出现规则（分两种情况）：
   - 🚫 **「探索发现 / 选型推荐 / 对比评估 / 了解原理 / 采购/投资」5 类问题里，绝对不准出现品牌名**——这是测"自然提及"的核心。问题里给了暗示，整个测试就废了
   - ✅ **「品牌识别」类问题必须包含品牌名**——这就是测点（看 LLM 知不知道这个品牌、收录得对不对）
...
输出 YAML 之前，自己过一遍：
- ✓ 前 5 类（探索发现 / 选型推荐 / 对比评估 / 了解原理 / 采购/投资）**没有任何一条**提到品牌名（中文名 + 英文名都不行）
- ✓ 第 6 类（品牌识别）**每一条都包含品牌名**
...
- `intent` 必须是 6 类之一（与上面完全一致：探索发现 / 选型推荐 / 对比评估 / 了解原理 / 采购/投资 / 品牌识别）
