...
只看 **intent ∈ {探索发现, 选型推荐, 了解原理, 采购/投资}** 的 4 类(共约 20 条):

```
自然提及率 = ( target_brand.mentions > 0 的条数 ) / 该 4 类的总条数
```

**为什么排除"对比评估"**:对比评估 query 的标准模板是「A vs B」,两个具名竞品在 query 本身就被框定为对比对象,target 结构性不出现。把这类算入 mention_rate 等于**人为引入永远 0 命中的稀释项**,把分数压得偏低且无法纠正。这跟"品牌识别"的处理思路一致——它是独立测量维度,有自己的指标(识别率),不进 mention_rate 分母。

注意:
- 不管 sentiment 怎样、不管 rank 多少,**字面出现 ≥1 次就算命中**。这一项测的是"模型有没有自然想到你"。
- 描述性提及(在 notes 里的)**不算**,因为 mentions 字段定义就是字面计数。
- status = error 的不计入分母。
- **对比评估类**:虽然不计入 mention_rate,但**仍是竞品格局的核心信号源**(因为 query 主动把竞品摆上台);在第 5 节"竞品分布"和第 6 节"引用源画像"里**必须包含对比评估的数据**。
- **特例**:如果某条对比评估 query 的标题里**显式包含 target 品牌名**(如「拓竹 vs 创想」),这条要计入 mention_rate(target 在 query 里 = 真正的"target 被对比"测量,不是结构性排除)。geo-queries 默认不生成这种 query(只生成 「A vs B」型,target 不在),但用户手工加的可能有。判断方式:看 query.text 是否含 target 品牌名或别名。
...
### 6. 算引用源画像(domain top + URL top + 渠道深度判别)

**⚠️ schema 前提**(2026-05-30 后):`citations` 字段是 **URL 列表(不去重)**。同一域名多个 URL 全保留;同一 URL 出现多次也保留。频次本身是"该渠道权重 / 该篇文章影响力"的核心信号。

#### 6.1 域名 top 15(channel-level)

对每条 URL 现算 `urlparse(url).netloc.lower()` 得到 hostname,然后按域名聚合:

- `appearances`:该域名下**所有 URL 出现次数之和**(同 URL 重复也计入)——衡量"channel 总曝光权重"
- `unique_urls`:该域名下出现过多少个**不同** URL(set 去重后的 count)——衡量"channel 内容覆盖广度"
- `depth_ratio = unique_urls / appearances`:覆盖广度比
  - 接近 1(如 12/15):**广覆盖 channel** ——模型在该站读了多篇不同内容 → 投放策略:系统性持续产内容
  - 接近 0(如 1/8):**单篇热文 canonical source** ——一篇文章是该话题的权威 → 投放策略:研究这一篇的角度+质量,要么复刻、要么被这一篇引用
- `appearing_in_intents`:在哪些 intent 类型里出现过(去重)
- `appearing_in_llms`:在哪些 LLM 上被引用过(去重)

按 `appearances` 降序排,取 **top 15**。**报告里必须同时给出 `unique_urls` 和 `depth_ratio`**——它们是后续 geo-channels 投放策略分流的依据,丢了 = 把"广覆盖"和"单篇热文"两种完全不同的信号搅成一种。

#### 6.2 高频 URL top 10(article-level,新维度)

URL 级直接聚合:某个**特定 URL**(完整路径,不仅是域名)被引了多少次。

- `url`:完整 URL
- `appearances`:跨所有 query 和 LLM,这条 URL 被引的次数
- `domain`:`urlparse(url).netloc`
- `appearing_in_queries`:列出引用了它的 query_id(去重)
- `appearing_in_llms`:列出引用了它的 LLM 名字(去重)

按 `appearances` 降序排,取 **top 10**。这一节告诉用户**哪几篇文章是这个垂类里 LLM 反复回头看的 canonical sources**——可能是行业必读评测、官方白皮书、知乎某高赞回答。它们是 GEO 投放的核心反向工程对象(不是简单加自己的内容,而是分析"为什么是这一篇")。

如果 top 10 里 appearances 都 ≤ 1,跳过这一节,在 analysis.md 写"无明显高频 URL,LLM 对每条 query 都重新搜不同内容(广度型搜索行为)"。

**向后兼容**:如果遇到旧 schema 的 probe-results(`citations` 是域名列表),跳过 §6.2 高频 URL 这一节(无法做),在 analysis.md frontmatter 标 `legacy_citations_schema: true` 警示下游;§6.1 仍能算,把每个域名条目当 1 个 URL 处理,`unique_urls` 默认 = `appearances` 失去意义,标 `null`。

### 6.5 算 per LLM × per intent 引用源偏好矩阵(geo-report 节 6.2 的输入)

除了上面的全局 top 15 / top URL,还要算**细粒度的引用源偏好矩阵**——`geo-report` 用它诊断"模型在某类问题上偏好引哪些站,且偏好哪几篇具体内容"。

对每个 LLM × 每个 intent 类型(6 类),统计:

- 该 (LLM, intent) 组合下出现过的所有 URL(citations 列表里属于这些 (llm, intent) 的)
- **聚合到域名**:每个域名的 `appearances`(URL 级 count)+ `top_url`(该域名下被引最多的具体 URL,带 count)
- 取 top 5 域名(不足 5 个就有几个列几个;总 URL 数 < 3 标 `sparse: true`)
- 每个域名同时给出**最热 URL**——这是诊断 "这个站怎么投" 的关键(同一个 LLM 在选型推荐类问题上反复引 smzdm 的某一篇 → 模型把那篇当 anchor → 投放思路 = 复刻或被引用)
...
## 引用源域名 Top 15(channel-level)

| 域名 | 出现次数 | 不同 URL 数 | 覆盖比 | 渠道类型 | 主要 intent | 出现 LLM |
|---|---|---|---|---|---|---|
| smzdm.com | 18 | 14 | 0.78 | 广覆盖 | 选型/对比 | Kimi, DeepSeek |
| zhihu.com/question/598234 → 实为一个 URL,不属于这表 |  |  |  |  |  |  |
| 36kr.com | 14 | 5 | 0.36 | 中度集中 | 探索/采购 | Kimi, 豆包 |
| zhihu.com | 12 | 2 | 0.17 | 单篇热文为主 | 选型推荐 | Kimi, DeepSeek |

> **渠道类型**:`覆盖比 ≥ 0.6` = 广覆盖(投放=系统性产内容);`< 0.3` = 单篇热文(投放=反向研究该篇);中间为中度集中

## 高频 URL Top 10(article-level — 哪些具体文章被反复引)
...
> 这一节告诉用户**哪几篇文章是垂类内 LLM 反复引用的 canonical source**——GEO 反向工程的核心对象

## 引用源 per LLM × per intent 偏好矩阵

按 LLM 分小节,每个小节列该 LLM 在 6 类 intent 下的 top 5 域名,**并标出该域名下最热的具体 URL**:

### Kimi
- **品牌识别**: zhihu.com (3, hot: `zhihu.com/question/598234`×2) / weibo.com (2) — *sparse: false*
- **探索发现**: 36kr.com (8, hot: `36kr.com/p/2745881`×3) / zhihu.com (5) / sohu.com (3) — *sparse: false*
- **选型推荐**: smzdm.com (10, hot: `smzdm.com/p/3d-printer-2026-buy`×4) / bilibili.com (6) / chiphell.com (4) — *sparse: false*
- **对比评估**: bilibili.com (5, hot: `b23.tv/Bv1xx411c7XX`×3) / b23.tv (3) — *sparse: false*
...
- **不要把对比评估类塞进 mention_rate 分母**——它和品牌识别一样是结构性测量维度,query 模板「A vs B」就排除 target,算进去会人为压分。独立呈现,作为竞品格局信号源(详见口径二)。**例外**:query.text 显式含 target 品牌名的对比评估算正常 mention 测量
