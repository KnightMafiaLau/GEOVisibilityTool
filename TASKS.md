...
- [x] **模块 4：`geo-report`** — 把 `analysis-<brand>-<date>.md` 渲染成给决策人看的对外可读报告 `report-<brand>-<date>.md`。顾问咨询语调,7 节结构,partial 模式强红 banner,竞品反向估算横向对比表(只比单维指标、不造合成分),per LLM × per intent 引用源偏好诊断,零命中 top 10 全展开。**不出"下一步建议 / 投放方案"——边界严格止于诊断**,投放策略留给 geo-channels。纯 skill。
